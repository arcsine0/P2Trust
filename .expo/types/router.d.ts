...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(login)` | `/(login)/` | `/(login)/register` | `/(tabs)` | `/(tabs)/` | `/(tabs)/history` | `/(tabs)/settings` | `/(transactionRoom)` | `/(transactionRoom)/` | `/(transactionRoom)/scan` | `/(transactionRoom)/verify` | `/..\components\chatEvents\TimelineEvents` | `/_sitemap` | `/history` | `/register` | `/scan` | `/settings` | `/verify`;
