...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(login)` | `/(login)/` | `/(login)/registerIdentifier` | `/(login)/registerPass` | `/(tabs)` | `/(tabs)/` | `/(tabs)/history` | `/(tabs)/settings` | `/(transaction)` | `/(transaction)/` | `/(transaction)/lobby` | `/(transaction)/scan` | `/..\lib\context\UserContext` | `/..\lib\helpers\collections` | `/..\lib\helpers\types` | `/..\lib\types\types` | `/_sitemap` | `/history` | `/lobby` | `/registerIdentifier` | `/registerPass` | `/scan` | `/settings`;
